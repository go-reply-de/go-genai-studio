...
const { STORAGE_BASE_URL } = require('./initialize');
const { saveBufferToGCS } = require('./crud');
 ...
 */
async function uploadImageToGCS({ req, file, file_id, endpoint, resolution = 'high' }) {
  ...
  const downloadURL = await saveBufferToGCS({ userId, buffer: webPBuffer, fileName });
...
/**
 * GCS: For GCS it only need to update filepath by
 * replacing the http://storage.cloud.google.com/ with gs://
 ...
 */
async function prepareImageGCS(req, file) {
  const { filepath } = file;

  const gsFilePath = filepath.replace(STORAGE_BASE_URL, 'gs://');
  // In the local strategy, the file object is updated with the passed file object.
  // No clue why this is relevant, so I'm just going to return the file object.
  //promises.push(updateFile({ file_id: file.file_id }));
  //promises.push(filepath);
  const promises = [];
  promises.push(file);
  promises.push(gsFilePath);
  return await Promise.all(promises);
}

/**
 * Uploads a user's avatar to GCS Storage and returns the URL.
 ...
 * @throws {Error} - Throws an error if GCS is not initialized or if there is an error in uploading.
 */
async function processGCSAvatar({ buffer, userId, manual }) {
  try {
    const downloadURL = await saveBufferToGCS({
      userId,
      buffer,
      fileName: 'avatar.png',
    });
    ...
    // relax error throwing for avatar upload
    // due to the fact that avatar upload is not a critical operation and
    // we obtain the refreshToken afterwards in the process
    logger.warn('Error processing GCS avatar:', error);
  }
}

module.exports = { uploadImageToGCS, prepareImageGCS, processGCSAvatar };
