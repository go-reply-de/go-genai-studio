const { EModelEndpoint, CacheKeys, Constants, goreplySettings } = require('librechat-data-provider');
...
const addTitle = async (req, { text, response, client }) => {
  ...
  const providerConfig = req.app.locals[EModelEndpoint.goreply];
  let model =
    providerConfig?.titleModel ??
    GOOGLE_TITLE_MODEL ??
    client.options?.modelOptions.model ??
    goreplySettings.model.default;
  ...
  await saveConvo(
    ...
    { context: 'api/server/services/Endpoints/goreply/addTitle.js' },
  );
};
